...
- 自反性 (Reflexive)：
    ...
    1. 要求：必须覆盖集合 $A$ 中的所有元素（矩阵主对角线全为 1，图上每个节点都有自环）
    2. 判断示例：若 $A = \{1, 2, 3\}$，关系 $\{(1,1), (1,2), (2,2)\}$ 不是自反的（缺少 $(3,3)$）

- 对称性 (Symmetric)：

    $$(a, b) \in R \land a \neq b \Rightarrow (b, a) \in R \quad (\text{等价于: } (a, b) \in R \Rightarrow (b, a) \in R)$$

    1. 要求：若存在一条有向边，则必存在反向边（逻辑矩阵是对称矩阵 $M = M^T$，可由无向图表示）
    2. 判断示例：$\{(2,1), (1,2), (2,2)\}$ 是对称的；而 $\{(1,1), (1,2), (2,2)\}$ 不是对称的（有 $(1,2)$ 却无 $(2,1)$）

- 反对称性 (Antisymmetric)：

    $$(a, b) \in R \land a \neq b \Rightarrow (b, a) \notin R \quad (\text{等价形式: } (a, b) \in R \land (b, a) \in R \Rightarrow a = b)$$

    1. 要求：两个不同节点之间至多只能有一条单向边，不允许存在双向箭头
    2. 判断示例：$\{(1,1), (1,2), (2,2)\}$ 是反对称的；而 $\{(1,2), (2,1), (2,2)\}$ 不是反对称的

- 传递性 (Transitive)：

    $$(a, b) \in R \land (b, c) \in R \Rightarrow (a, c) \in R$$

    1. 要求：若存在两步连续路径 $a \to b \to c$，则必存在一步直达边 $a \to c$
    2. 判断示例：
        - $\{(1,1), (1,2), (2,3)\}$ 不是传递的（有 $(1,2)$ 和 $(2,3)$，但缺少 $(1,3)$）
        - $\{(1,2), (1,3)\}$ 是传递的（不存在连续的两步路径前件，真空成立 (Vacuously True)）

- 对称与反对称的关系辨析：
    1. 对称与反对称绝不是互斥的互补对立关系
...
- 等价关系 (Equivalence Relation)：
    1. 判定条件：同时满足自反 (Reflexive)、对称 (Symmetric)、传递 (Transitive) 的二元关系
    2. 图示特征：图由若干个互不相连的团簇 (Clusters) 构成，每个簇内部的所有节点两两相互连通

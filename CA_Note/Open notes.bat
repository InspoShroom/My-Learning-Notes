@echo off
title CA Notes Server
echo ===================================================
echo   CA Notes Startup and Management Service
echo ===================================================
echo.

:: 1. Clean port 8124
echo [1/4] Checking and cleaning port 8124...
set found=0
for /f "tokens=5" %%a in ('netstat -ano ^| findstr :8124 ^| findstr LISTENING') do (
    taskkill /F /PID %%a >nul 2>&1
    set found=1
)
if %found%==1 (
    echo      Successfully terminated conflicting process on port 8124.
) else (
    echo      Port 8124 is clean and free.
)
...
echo      -------------------------------------------------
echo.
python -m mkdocs serve -a 127.0.0.1:8124
